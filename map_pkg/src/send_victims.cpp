...
public:
  ...
  on_configure(const rclcpp_lifecycle::State& state)
  {
    ...
    this->publisherm_ = this->create_publisher<visualization_msgs::msg::MarkerArray>("/markers/victims", this->qos);
...
void VictimPublisher::spawn_and_publish_victims(const std::vector<obstacle>& victims)
{
  ...
  for (auto vict : victims) {
    ...
    if (this->get_parameter("use_gui").as_bool()) {
      spawn_model(this->get_node_base_interface(), this->spawner_, xml_string, pose, "victim", false);
    }
    ...
    mark.pose = pose;
    mark.scale.x = obs.radius/this->data.max_weight;
    mark.scale.y = obs.radius/this->data.max_weight;
    mark.scale.z = 0.01;
    mark.color.a = 1.0;
    mark.color.r = 0.0;
    mark.color.g = 0.0;
